...
function UPS(args) {
	var $scope = this;
	this.freight_billing_options = staticValues.freight_billing_options;
	this.freight_service_classes = staticValues.freight_service_classes;
	this.freight_service_codes = staticValues.freight_service_codes;
	this.freight_handling_codes = staticValues.freight_handling_codes;
	this.freight_package_codes = staticValues.freight_package_codes;
	this.pickup_package_types = staticValues.pickup_package_types;

	this.options = staticValues.defaults;
	this.service_codes = staticValues.default_services;
	this.tracking_codes = staticValues.tracking_status_codes;
	...
	function buildShipment(data, options) {
		...
		if(data.ship_from) {
			...
			if(data.ship_from.tax_id_type && staticValues.tax_id_types[data.ship_from.tax_id_type]) {
				shipment.ShipFrom.TaxIDType = { Code: data.ship_from.tax_id_type, Description: staticValues.tax_id_types[data.ship_from.tax_id_type] };
			}
		}
		...
		if(data.service && staticValues.default_services[data.service]) {
			shipment['Service'] = {
				'Code': data.service
			};
		}

		if(data.return_service && staticValues.return_services[data.return_service]) {
	...
	function buildLabelSpecification(data, options) {
		var label_type = data.label_type || $scope.options.label_type;
		var label;
		if(label_type === 'EPL') {
			label = {
				'LabelPrintMethod': {
	...
	function doBuildParams(data, options, resource) {
		...
		var body = callBody;
		var params = {
			host: staticValues.hosts[$scope.options.environment],
			path: resource.p,
			method: 'POST',
			headers: {
	...
	function doRequest(params, body, callback) {
		...
		req.on('response', function(res) {
			...
			res.on('end', function() {
				...
				var json;
				try {
					json = parser.toJson(responseData, {coerce: false, object: true, sanitize: false});
				} catch(e) {
	...
	function density(weight, length, width, height) {
		return (weight / ((length * width * height) / 1728));
	}

	function dimensionalWeight(imperial, weight, length, width, height) {
		var dimWeight = (length * width * height) / (imperial ? staticValues.dimensional_weight_values.imperial : staticValues.dimensional_weight_values.metric);
		if(dimWeight > weight) {
	...
	$scope.dimensionalWeight = function(weight, length, width, height) {
		return dimensionalWeight($scope.options.imperial, weight, length, width, height);
	};
	...
	$scope.buildRatesRequest = function(data, options) {
		...
		var request = {
			...
			'PickupType': {
				'Code': data.pickup_type_code || staticValues.pickup_codes[data.pickup_type]
			},
	...
	$scope.buildShipmentConfirmRequest = function(data, options) {
		...
		request['Shipment']['Service'] = {
			...
			'Description': staticValues.default_services[data.service_code] || staticValues.default_services['03']
		};
	...
	$scope.buildTimeInTransitRequest = function(data, options) {
		...
		var tomorrow = String(date.getFullYear()) +
	...
	$scope.buildPickupRequest = function(data, options, resource, callback) {
		soap.createClient(path.join(__dirname,	'wsdl', 'pickup', 'Pickup.wsdl'), {endpoint: 'https://' + staticValues.hosts[$scope.options.environment] + resource.p}, function(err, client) {
			if(err) {
				return callback(err, null);
			}
	...
	$scope.buildCancelPickupRequest = function(data, options, resource, callback) {
		soap.createClient(path.join(__dirname,	'wsdl', 'freight_pickup', 'FreightPickup.wsdl'), {endpoint: 'https://' + staticValues.hosts[$scope.options.environment] + resource.p}, function(err, client) {
			if(err) {
				return callback(err, null);
			}
	...
	$scope.buildFreightRateRequest = function(data, options, resource, callback) {
		soap.createClient(path.join(__dirname,	'wsdl', 'freight_rate', 'FreightRate.wsdl'), {endpoint: 'https://' + staticValues.hosts[$scope.options.environment] + resource.p}, function(err, client) {
			if(err) {
				return callback(err, null);
			}
	...
	$scope.buildFreightShipRequest = function(data, options, resource, callback) {
		...
		var header = '<?xml version="1.0" encoding="UTF-8"?><soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:error="http://www.ups.com/XMLSchema/XOLTWS/Error/v1.1" xmlns:upss="http://www.ups.com/XMLSchema/XOLTWS/UPSS/v1.0" xmlns:tns="http://www.ups.com/WSDL/XOLTWS/FreightShip/v1.0">';
			...
			doRequest({host: staticValues.hosts[$scope.options.environment],
				path: resource.p,
				method: 'POST',
				headers: {
					'Content-Length': body.length,
	...
	this.options = extend(staticValues.defaults, args);
	return this;
}
